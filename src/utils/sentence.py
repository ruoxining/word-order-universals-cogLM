...
class PhraseStructureSentence(object):
    ...
    def __init__(self, sent_line, has_preterms=True):
        ...
        self._dfs_traversal_simple = None
    ...
    def _dfs_traverse_recursive_simple(self, tree, output):
        """A recursive function that actually does the depth-first traversal.
        ...
        if isinstance(tree, (str)):  # Terminal symbol
            ...
            if sym_type == "PRETERM":  # Preterminals can only have one child.
                assert len(tree) == 1
            if len(tree) == 1 and tree.height() > 2 and tree.label() != "ROOT" and tree.label() == tree[0].label():
                sym_type = "UNNECESSARY"
            else:
                output.append((sym_type, tree.label()))
            for subtree in tree:
                self._dfs_traverse_recursive_simple(subtree, output)
            if sym_type == "NT":
                output.append(("REDUCE", tree.label()))
        return output

    def dfs_traverse(self, simplify=False):
        ...
        # self._dfs_traversal.
        if simplify:
            if self._dfs_traversal_simple is not None:
                yield from self._dfs_traversal_simple
            else:
                # Split the sentence to iterate over the symbols.
                output = []
                self._dfs_traverse_recursive_simple(self._sent_tree, output)
                self._dfs_traversal_simple = output  # Cache the value
                for sym in output:
                    yield sym
    ...
    def _lc_traverse_arc_standard_recursive(self, tree, output):
        ...
        if isinstance(tree, (str)):  # Terminal symbol
            ...
            if tree.height() == 2 and self.has_preterms:
                sym_type = "PRETERM"
            if sym_type == "NT":
                self._lc_traverse_arc_standard_recursive(tree[0], output)
            output.append((sym_type, tree.label()))
            ...
            if sym_type == "PRETERM":  # Preterminals can only have one child.
                ...
                for subtree in tree[1:]:
                    self._lc_traverse_arc_standard_recursive(subtree, output)
                output.append(("REDUCE", tree.label()))

        return output

    def lc_traverse_arc_standard(self):
        ...
        # Split the sentence to iterate over the symbols.
        output = []
        self._lc_traverse_arc_standard_recursive(self._sent_tree, output)
        for sym in output:
            yield sym
    ...
    def bu_traverse(self):
        ...
        # output.append(("STOP", None)) # TODO: I don't know why this is needed only in bu traversal
        for sym in output:
            yield sym

    def get_action(self, sym_type, symbol, output, gen_preterm=False, untyped_closing_terminal=False):
        if sym_type == "PRETERM":
            if gen_preterm:
    ...
    def convert_to_choe_charniak(self, untyped_closing_terminal=True, gen_preterm=False, simplify=False):
        ...
        """
        output = []
        for sym_type, symbol in self.dfs_traverse(simplify=simplify):
            output = self.get_action(sym_type, symbol, output, gen_preterm, untyped_closing_terminal)
