...
def postprocess_token_ids(ids: Sequence[int], vocab: sp_utils.SentencePieceVocab) -> Iterable[int]:
    ...
    for id_ in ids:
        if between_words:
            if vocab.is_whitespace(id_):
                ...
                logging.warning(
                    "Encountered token %d which is neither a terminal or a " "non-terminal. UNK? Skipping.", id_
                )
        else:
            if vocab.is_whitespace(id_):
                new_ids.append(id_)
...
def choe_charniak_from_tree(
    s: str, has_preterms: bool = True, untyped_closing_terminal: bool = False, simplify: bool = False
):
    ...
    return sent.convert_to_choe_charniak(untyped_closing_terminal, simplify=simplify)
