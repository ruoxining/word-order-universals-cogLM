...
class RNNLM(nn.Module):
    def __init__(self, vocab=10000, w_dim=650, h_dim=650, num_layers=2, dropout=0.5):
        super(RNNLM, self).__init__()
        self.h_dim = h_dim
        self.num_layers = num_layers
        self.word_vecs = nn.Embedding(vocab, w_dim)
        self.dropout = nn.Dropout(dropout)
        self.rnn = nn.LSTM(w_dim, h_dim, num_layers=num_layers, dropout=dropout, batch_first=True)
...
class SeqLSTM(nn.Module):
    def __init__(self, i_dim=200, h_dim=0, num_layers=1, dropout=0):  # word embedding dim  # internal hidden dim
...
class TopDownRNNG(nn.Module):
    def __init__(
        ...
        self.composition = (
            AttentionComposition(w_dim, dropout, self.action_dict.num_nts())
            if attention_composition
            else LSTMComposition(w_dim, dropout)
        )
    ...
    def word_sync_beam_search(
        self, x, beam_size, word_beam_size=0, shift_size=0, delay_word_ll=False, return_beam_history=False
    ):
        self.eval()
            ...
            while not all(len(batch_beam) == 0 for batch_beam in beam):
                ...
                for b in range(len(beam)):
                    if len(word_completed[b]) - forced_completions[b] >= beam_size:
                        beam[b] = []
                if return_beam_history:
                    beam_histroy.append((pointer, bucket_i, [b[:] for b in beam], [b[:] for b in word_completed]))
                bucket_i += 1
        ...
        parses = [[(item.parse_actions(), item.score) for item in batch_beam] for batch_beam in beam]
    ...
    def variable_beam_search(self, x, K, original_reweight=False):
        ...
        for pointer in range(x.size(1) + 1):
            ...
            for b in range(len(beam)):
                beam[b] = self.reweight_and_filter_particles(word_completed[b], K, original_reweight=original_reweight)
                word_completed[b] = []
        ...
        parses = [
            sorted([(item.parse_actions(), item.score) for item in batch_beam], key=lambda x: x[1], reverse=True)
    ...
    def _initial_hs(self, x, initial_stack=None):
        ...
        return [[(stack_layer[0][b], stack_layer[1][b]) for stack_layer in initial_stack] for b in range(x.size(0))]
    ...
    def get_successors(self, x, pointer, beam, beam_size, shift_size, delay_word_ll=False):
        ...
        if delay_word_ll:
            ...
            successors = [
                [
                    beam[batch_i][b].next_incomplete_item(a, s, local_gen_probs[batch_i][b][a])
            ...
            successors = [
                [beam[batch_i][b].next_incomplete_item(a, s) for (b, a, s) in batch_succs]
                for batch_i, batch_succs in enumerate(successors)
            ]
    ...
    def get_successors_by_particle_filter(self, x, pointer, beam):
        ...
        log_probs, disc_log_probs = self.action_log_probs(states, action_mask, next_x, return_disc_probs=True)
        new_K = (
            disc_log_probs.exp() * log_probs.new_tensor([item.particle_path.K for item in all_items]).unsqueeze(1)
        ).round_()
        new_K = new_K.view(-1)
        ...
        for batch_i, beam_length in enumerate(beam_lengths):
            ...
            successors.append(
                [
                    beam[batch_i][beam_id[i]].next_incomplete_item(
    ...
    def reweight_and_filter_particles(self, batch_beam, K, upper_lex_size=1000, original_reweight=False):
        if original_reweight:
            scores = torch.tensor(
                [[b.particle_path.K, b.particle_path.gen_ll, b.particle_path.disc_ll] for b in batch_beam]
            )
    ...
    def valid_action_mask(self, items, sent_len):
        ...
        for b, item in enumerate(items):
            ...
            if state.pointer == sent_len:  # only reduce
                ...
                self.action_dict.mask_shift(mask, b)
            if state.nopen_parens > self.max_open_nts or state.ncons_nts >= self.max_cons_nts:  # no more open
                self.action_dict.mask_nt(mask, b)

            if (
                (state.nopen_parens == 1 and state.pointer < sent_len)
                or self.action_dict.is_nt(prev_action)  # cannot reduce the unique open element at intermediate steps.
                or len(state.stack) < 3  # cannot reduce immediately after nt
            ):
                self.action_dict.mask_reduce(mask, b)
    ...
    def scores_to_successors(self, x, pointer, beam, total_scores, beam_size, shift_size):
        ...
        for batch, batch_total_scores in enumerate(total_scores):
            ...
            successors[batch] = [
                (beam_id_np[i], action_id_np[i], sorted_scores_np[i]) for i in range(min(len(beam_id_np), beam_size))
            ]
            if pointer < x.size(1):
                ...
                if shift_in_successors < shift_size:
                    ...
                    additional = (
                        (action_id[beam_size:] == self.action_dict.a2i["SHIFT"])
                        .nonzero()[: shift_size - shift_in_successors]
                        .squeeze(1)
                    ).cpu()
                    additional += beam_size  # add offset
                    assert all(action_id_np[i] == self.action_dict.a2i["SHIFT"] for i in additional)
                    successors[batch] += [(beam_id_np[i], action_id_np[i], sorted_scores_np[i]) for i in additional]
                    forced_completions[batch] = additional.size(0)
            else:
                # At the end, save final actions.
                finish_actions = [
                    (b, a, s) for (b, a, s) in successors[batch] if self._is_last_action(a, beam[batch][b].state, True)
                ]
    ...
    def _is_last_action(self, action, state, shifted_all):
        return shifted_all and self.action_dict.is_reduce(action) and state.can_finish_by_reduce()
...
class TopDownState:
    def __init__(
        self, pointer=0, stack=None, stack_trees=None, nopen_parens=0, ncons_nts=0, nt_index=None, nt_ids=None
    ):
...
class BeamItem:
    ...
    def dump(self, action_dict, sent):
        ...
        stack_str = action_dict.build_tree_str(actions, sent.orig_tokens, ["" for _ in range(len(sent.orig_tokens))])
