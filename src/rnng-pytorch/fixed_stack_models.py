...
class MultiLayerLSTMCell(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bias=True, dropout=0, use_elman=False):
        super(MultiLayerLSTMCell, self).__init__()
        self.lstm = nn.ModuleList()
        if use_elman:
            self.lstm.append(nn.RNNCell(input_size, hidden_size))
            for i in range(num_layers - 1):
                self.lstm.append(nn.RNNCell(hidden_size, hidden_size))
        else:
        ...
        self.dropout_layer = nn.Dropout(dropout, inplace=False)
...
class LSTMComposition(nn.Module):
    def __init__(self, dim, dropout, use_elman=False):
        super(LSTMComposition, self).__init__()
        self.dim = dim
        if use_elman:
            self.rnn = nn.RNN(dim, dim, bidirectional=False, batch_first=True)
            self.output = nn.Sequential(nn.Dropout(dropout, inplace=True), nn.Linear(dim, dim), nn.ReLU())
        else:
    ...
    def forward(self, children, ch_lengths, nt, nt_id, stack_state):
        ...
        if self.use_elman:
            c = h.gather(1, gather_idx).squeeze(1)
        else:
...
class AttentionComposition(nn.Module):
    def __init__(self, w_dim, dropout, num_labels=10):
        ...
        self.nt_emb2 = nn.Sequential(
            nn.Embedding(num_labels, w_dim * 2), self.dropout
...
class FixedStack(ExpandableStorage):
    def __init__(self, initial_hidden, stack_size, input_size, beam_size=1, use_elman=False):
        ...
        if beam_size == 1:
            ...
            self.batch_index = (
                ...
                torch.cat([torch.arange(0, beam_size, dtype=torch.long, device=device) for _ in range(batch_size[0])]),
            )
        ...
        self.hiddens = initial_hidden[0].new_zeros(
            batch_size + (stack_size + 1, hidden_size, num_layers), device=device
        )
        self.use_elman = use_elman
        ...
        self.nt_index_pos = (
            torch.tensor([-1], dtype=torch.long, device=device).expand(batch_size).clone()
        )  # default is -1 (0 means zero-dim exists)

        self.attrs = ["pointer", "top_position", "hiddens", "cells", "trees", "nt_index", "nt_ids", "nt_index_pos"]
    ...
    def reset_batch_index(self):
        ...
        self.batch_index = (
            ...
            torch.cat(
                [torch.arange(0, self.beam_size, dtype=torch.long, device=device) for _ in range(self.batch_size)]
            ),
        )
    ...
    def update_hidden(self, new_hidden, new_cell=None):
...
class StackState(ExpandableStorage):
    def __init__(self, batch_size, beam_size, device, use_elman=False):
...
class ParticleAdditionalScores(ExpandableStorage):
    def __init__(self, batch_size, beam_size, initial_K, device):
        ...
        self.disc_ll = torch.tensor([-float("inf")], device=device).expand(batch_size, beam_size).clone()
        self.K = torch.tensor([initial_K], dtype=torch.float, device=device).expand(batch_size, beam_size).clone()
        self.disc_ll[:, 0] = 0
...
class BeamItems(ExpandableStorage):
    def __init__(self, stack, stack_state, max_actions=500, beam_is_empty=False, particle_filter=False, initial_K=0):
        ...
        self.gen_ll = (
            torch.tensor([-float("inf")], device=stack.hiddens.device).expand(self.batch_size, self.beam_size).clone()
        )
        self.gen_ll[:, 0] = 0
        ...
        self.action_path = ActionPath(self.batch_size, self.beam_size, max_actions, self.beam_widths.device)
    ...
    def shrink(self, size=-1):
        ...
        outside_beam_idx = (
            torch.arange(self.beam_size, device=self.gen_ll.device).unsqueeze(0) >= self.beam_widths.unsqueeze(1)
    ...
    def move_items_from(self, other, move_idxs, new_gen_ll=None, additional=(), allow_expand=False):
        """
        ...
        if new_beam_widths.max() >= self.beam_size:
            ...
            move_idx_mask = move_idx_mask.view(-1)[(beam_idx_order < batch_numbers.unsqueeze(1)).view(-1)]
    ...
    def _do_move_elements(self, source, self_idxs, source_idxs, new_gen_ll=None, new_additional=()):
...
class RNNGCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, dropout, action_dict, attention_composition, use_elman):
        ...
        self.stack_rnn = MultiLayerLSTMCell(input_size, hidden_size, num_layers, dropout=dropout, use_elman=use_elman)
        self.output = nn.Sequential(self.dropout, nn.Linear(hidden_size, input_size), nn.ReLU())
        self.composition = (
            AttentionComposition(input_size, dropout, action_dict.num_nts())
            if attention_composition
            else LSTMComposition(input_size, dropout, use_elman=use_elman)
        )
    ...
    def forward(self, word_vecs, actions, stack, subword_end_mask=None):
        ...
        if self.use_elman:
            new_hidden, _ = self.stack_rnn(new_input.view(-1, self.input_size), (stack.hidden_head(1), None))
            stack.update_hidden(new_hidden)
        else:
            new_hidden, new_cell = self.stack_rnn(
                new_input.view(-1, self.input_size), (stack.hidden_head(1), stack.cell_head(1))
            )
...
class FixedStackRNNG(nn.Module):
    def __init__(
        ...
        self.action_criterion = nn.CrossEntropyLoss(reduction="none", ignore_index=action_dict.padding_idx)
        self.word_criterion = nn.CrossEntropyLoss(reduction="none", ignore_index=padding_idx)

        self.dropout = nn.Dropout(dropout, inplace=True)
        self.emb = nn.Sequential(nn.Embedding(vocab, w_dim, padding_idx=padding_idx), self.dropout)

        self.rnng = RNNGCell(
            w_dim, h_dim, num_layers, dropout, self.action_dict, attention_composition, use_elman=use_elman
        )
    ...
    def build_stack(self, x, stack_size=80):
        ...
        return FixedStack(initial_hidden, stack_size, self.input_size, use_elman=self.use_elman)
    ...
    def word_sync_beam_search(
        ...
        if (
            hasattr(self.rnng.composition, "batch_index")
            and self.rnng.composition.batch_index.size(0) < x.size(0) * beam_size
        ):
        ...
        beam, word_completed_beam = self.build_beam_items(x, beam_size, shift_size, stack_size_bound=stack_size_bound)
        word_vecs = self.emb(x)
        ...
        for pointer in range(x.size(1) + 1):
            ...
            while not finished_batches.all():
                added_forced_completions = self.beam_step(
                    x, subword_end_mask, sent_lengths, word_vecs, pointer, beam, word_completed_beam, shift_size
                )
            ...
            self.finalize_word_completed_beam(
                x, subword_end_mask, sent_lengths, word_vecs, pointer, beam, word_completed_beam, word_beam_size
            )
    ...
    def beam_step(self, x, subword_end_mask, sent_lengths, word_vecs, pointer, beam, word_completed_beam, shift_size):
        beam_size = beam.beam_size
        successors, word_completed_successors, added_forced_completions = self.get_successors(
            x, subword_end_mask, sent_lengths, pointer, beam, beam_size, shift_size
        )
        ...
        if word_completed_successors[0][0].size(0) > 0:
            ...
            moved_idxs = word_completed_beam.move_items_from(beam, comp_idxs, new_gen_ll=word_completed_successors[2])
    ...
    def variable_beam_step(self, x, subword_end_mask, sent_lengths, word_vecs, pointer, beam, word_completed_beam, K):
        successors, word_completed_successors = self.get_successors_by_particle_filter(
            x, subword_end_mask, sent_lengths, pointer, beam
        )
        ...
        actions = successors[1].new_full((x.size(0), beam.beam_size), self.action_dict.padding_idx)
        actions[new_beam_idxs] = successors[1]
    ...
    def finalize_word_completed_beam(
        self, x, subword_end_mask, sent_lengths, word_vecs, pointer, beam, word_completed_beam, word_beam_size
    ):
    ...
    def build_beam_items(self, x, beam_size, shift_size, particle=False, K=0, stack_size_bound=100):
        # stack_size = max(100, x.size(1) + 10)
        if stack_size_bound <= 0:
            stack_size = max(100, x.size(1) + 10)
        else:
        ...
        initial_hidden = self.rnng.get_initial_hidden(x)
        stack_unfinished, state_unfinished = self.new_beam_stack_with_state(initial_hidden, stack_size, beam_size)
        ...
        return (
            BeamItems(stack_unfinished, state_unfinished, max_actions, False, particle, K),
            BeamItems(stack_word_finished, state_word_finished, max_actions, True, particle, K),
        )

    def new_beam_stack_with_state(self, initial_hidden, stack_size, beam_size):
        stack = FixedStack(initial_hidden, stack_size, self.input_size, beam_size, use_elman=self.use_elman)
    ...
    def variable_beam_search(self, x, subword_end_mask, K, original_reweight=False, stack_size_bound=100):
        ...
        if (
            hasattr(self.rnng.composition, "batch_index")
            and self.rnng.composition.batch_index.size(0) < x.size(0) * max_beam_size
        ):
            # The maximum number may be set by assuming training setting only.
            # Here we reset to the maximum number by beam search.
            self.rnng.composition.batch_index = torch.arange(0, x.size(0) * max_beam_size, device=x.device)

        beam, word_completed_beam = self.build_beam_items(
            x, max_beam_size, 0, True, K, stack_size_bound=stack_size_bound
        )
        word_vecs = self.emb(x)
        word_marginal_ll = [[] for _ in range(x.size(0))]
        ...
        for pointer in range(x.size(1) + 1):
            bucket_i = 0
            while not (
                (beam.beam_widths == 0) + (word_completed_beam.beam_widths == word_completed_beam.beam_size - 1)
            ).all():
    ...
    def initial_states(self, x):
        initial_hidden = self.rnng.get_initial_hidden(
            x
        )  # [(batch_size, hidden_size, layer), (batch_size, hidden_size, layer)]
        return [
            TopDownState.from_initial_stack((initial_hidden[0][b], initial_hidden[1][b])) for b in range(x.size(0))
        ]
    ...
    def get_successors_by_particle_filter(self, x, subword_end_mask, sent_lengths, pointer, beam):
        ...
        end_action_mask = ((pointer < sent_lengths).unsqueeze(1) * action_id == self.action_dict.a2i["SHIFT"]) + (
        ...
        def mask_to_successors(action_mask):
            ...
            return (
                (masked_idx[0], beam_id[masked_idx]),
        ...
        return (mask_to_successors(no_end_action_mask), mask_to_successors(end_action_mask))

    def finalize_particle_beam(
        self,
        x,
        subword_end_mask,
        sent_lengths,
        ...
        word_end_actions = x.new_full((x.size(0), beam.beam_size), self.action_dict.padding_idx)
    ...
    def reweight_and_filter_particles(self, beam, word_completed_beam, K, original_reweight=False):
        if original_reweight:
            # Try to avoid fp16 before logsumexp and exp, which may be sensitive to precision.
            log_weights = (
                word_completed_beam.additional.K.log().float()
    ...
    def invalid_action_mask(self, beam, sent_lengths, subword_end_mask):
        ...
        reduce_mask = (
            ((beam.nopen_parens == 1) * (beam.stack.pointer < sent_lengths))
            +
        ...
        nt_mask = (
            (beam.nopen_parens >= self.max_open_nts)
            ...
            (beam.actions.size(2) - beam.actions_pos < (sent_lengths - beam.stack.pointer + beam.nopen_parens + 3))
            +
    ...
    def scores_to_successors(self, x, sent_lengths, pointer, beam, total_scores, beam_size, shift_size):
        ...
        within_beam_num_end_actions = bincount_and_supply(within_beam_end_action_idx[0], x.size(0))  # (batch_size)
        ...
        num_to_be_forced_completions = torch.maximum(
            torch.tensor([0], device=x.device).expand(x.size(0)), shift_size - within_beam_num_end_actions
        )  # (batch_size)
        ...
        outside_end_action_idx = (outside_end_action_idx[0][filter_mask], outside_end_action_idx[1][filter_mask])
        ...
        end_successor_idx = torch.cat([end_action_mask[:, :beam_size], outside_end_action_mask], dim=1).nonzero(
            as_tuple=True
        )
