...
class Vocabulary(object):
    ...
    def __init__(self, w2c_list, pad="<pad>", unkmethod="unk", unktoken="<unk>", specials=[]):
        self.pad = pad
        self.padding_idx = 0
    ...
    def load(cls, fn):
        ...
        pad, unkmethod, unktoken, specials = [l.strip() for l in lines[:4]]
    ...
    def from_data_json(data):
        d = data["vocab"]
        return Vocabulary(d["word_count"], d["pad"], d["unkmethod"], d["unktoken"], d["specials"])
        ...
        def read_jsonl(f):
            ...
            for line in f:
                ...
                if k == "sentence":
                    ...
                    o["tokens"] = o["orig_tokens"] = o["tree_str"] = o["actions"] = o["in_order_actions"] = o[
                        "tags"
                    ] = None
                    sents.append(o)
                else:
    ...
    def from_text_file(
        ...
        with open(text_file) as f:
            for line in f:
                ...
                sents.append(sent)
        return Dataset(
            sents, batch_size, vocab, action_dict, False, prepro_args, batch_token_size, batch_group=batch_group
        )
    ...
    def batches_helper(self, len_to_idxs, shuffle=True, test=False):
        ...
        for length, idxs in len_to_idxs.items():
            ...
            for i in range(len(idxs)):
                ...
                if i > b and (  # for ensuring batch size 1
                    (longest_sent_len * (batch_i + 1) >= batch_token_size)
                    or (longest_action_len * (batch_i + 1) >= batch_action_size)
        ...
        for batch_idx in batches:
            ...
            if not test:
                ...
                ret += (torch.tensor(self._pad_action_ids(action_ids), dtype=torch.long), max_stack_size)
