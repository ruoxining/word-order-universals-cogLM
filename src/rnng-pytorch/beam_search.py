...
parser.add_argument(
    "--delay_word_ll", action="store_true", help="Adding shift word probability is delayed at word-synchronizing step"
)
...
parser.add_argument("--use_elman", action="store_true")
...
def main(args):
    ...
    if args.particle_filter:
        ...
        def parse(tokens, subword_end_mask, return_beam_history=False, stack_size_bound=-1):
            return model.variable_beam_search(
                tokens, subword_end_mask, args.particle_size, args.original_reweight, stack_size_bound=stack_size_bound
            )

    else:

        def parse(tokens, subword_end_mask, return_beam_history=False, stack_size_bound=-1):
            ret = model.word_sync_beam_search(
                tokens,
                subword_end_mask,
                args.beam_size,
                args.word_beam_size,
    ...
    with torch.no_grad():
        ...
        block_surprisals = (
            []
        )  # This is subword-based surprisal in general. Conversion to word-level surprisal is done at output phase.
        batches = [batch for batch in dataset.test_batches(args.block_size, max_length_diff=args.max_length_diff)]
        for batch in tqdm(batches):
            ...
            if any(len(p) == 0 for p in parses):
                ...
                failed_sents = [
                    (idx, " ".join(dataset.sents[idx].orig_tokens)) for p, idx in zip(parses, batch_idx) if len(p) == 0
                ]
                failed_sents_str = "\n".join("{}: {}".format(i, s) for i, s in failed_sents)
                logger.warning("Parse failure occurs on the following " "sentence(s):\n{}".format(failed_sents_str))
                if args.stack_size_bound < tokens.size(1):
                    ...
                    logger.warning(
                        "Current stack size bound {} is smaller than "
                        "sentence length {}.".format(args.stack_size_bound, tokens.size(1))
                    )
                    logger.warning("Try to reparse with increased stack size bound...")
                    parses, surprisals, beam_history = try_parse(tokens, subword_end_mask, stack_size_bound=-1)
        ...
        logger.info(
            {
                ...
                "word_beam_size": args.word_beam_size,
    ...
    logger.info(
        {
            ...
            "data": args.test_file.split("/")[-4],
            "lang": args.test_file.split("/")[-3],
            "fold": args.test_file.split("/")[-1].split("-")[0],
            "save_path": args.model_file,
            "test_data_size": args.test_file.split(".")[-1],
        }
    )
