...
parser.add_argument("--use_elman", action="store_true", help="use simple Elman RNN instead of LSTM")
...
parser.add_argument(
    "--plateau_lr_decay", default=0.5, type=float, help="lr is decayed at this rate if --lr_scheduler=plateau"
)
parser.add_argument(
    "--plateau_lr_patience",
    default=1,
...
parser.add_argument("--warmup_steps", default=10000, type=float, help="Total steps (batches) for warmup")
parser.add_argument(
    "--random_unk",
    action="store_true",
    help="Randomly replace a token to <unk> on training sentences by a probability inversely proportional to word frequency.",
)
...
parser.add_argument(
    "--batch_action_size", type=int, default=45000, help="(batch_size*max_action_length) does not exceed this."
)
...
parser.add_argument(
    "--min_epochs", default=8, type=int, help="do not decay learning rate for at least this many epochs"
)
...
parser.add_argument(
    "--valid_every", type=int, default=-1, help="If > 0, validate and save model every this many batches"
)
...
def create_scheduler(args, optimizer):
    ...
    if args.lr_scheduler == "plateau":
        return ReduceLROnPlateau(
            optimizer, factor=args.plateau_lr_decay, patience=args.plateau_lr_patience, verbose=True
        )
...
def create_model(args, action_dict, vocab):
    ...
    try:
        # use_elman optionがないモデルのロード用
        model_args.update({"use_elman": args.use_elman})
    except:
        model_args.update({"use_elman": False})
...
def main(args):
    ...
    val_data = Dataset.from_json(args.val_file, args.batch_size, vocab, action_dict, oracle=args.strategy)
    if args.do_test:
        test_data = Dataset.from_json(args.test_file, args.batch_size, vocab, action_dict, oracle=args.strategy)
    vocab_size = int(train_data.vocab_size)
    logger.info(
        "Train: %d sents / %d batches, Val: %d sents / %d batches"
        % (len(train_data.sents), len(train_data), len(val_data.sents), len(val_data))
    )
    ...
    def is_early_stop():
        return (
            ...
            and all([val_losses[-i - 2] < val_losses[-i - 1] for i in range(args.early_stop_patience - 1)])
        )
    ...
    if args.do_train:
        ...
        while epoch <= args.num_epochs and not is_early_stop():
            ...
            def calc_loss(token_ids, action_ids, max_stack_size, subword_end_mask):
                loss, a_loss, w_loss, _ = model(
                    token_ids, action_ids, stack_size_bound=max_stack_size, subword_end_mask=subword_end_mask
                )
            ...
            def try_batch_step(token_ids, action_ids, max_stack_size, subword_end_mask, num_divides=1):
                try:
                ...
                return try_batch_step(token_ids, action_ids, max_stack_size, subword_end_mask, num_divides * 2)
            ...
            for batch in train_data.batches():
                ...
                if b % args.print_every == 0:
                    ...
                    # wandb.log({"Epoch": continuous_epoch, "Train ppl": ppl, "Train word ppl": word_ppl, "lr": args.lr})
                    logger.info(
                        {"Epoch": continuous_epoch, "Train ppl": ppl, "Train word ppl": word_ppl, "lr": args.lr}
                    )
            ...
            if args.valid_every <= 0:
                do_valid(model, optimizer, scheduler, train_data, val_data, epoch, epoch, val_losses, args)

        # Last validation is necessary when validations were performed intermediately.
        if args.valid_every > 0:
            do_valid(model, optimizer, scheduler, train_data, val_data, epoch, continuous_epoch, val_losses, args)
        ...
        logger.info("Saving last checkpoint to {}".format(args.save_path))
...
def do_valid(model, optimizer, scheduler, train_data, val_data, epoch, step, val_losses, args):
    ...
    def consecutive_increase(target_losses):
        return len(target_losses) > 0 and all(
            target_losses[i] < target_losses[i + 1] for i in range(0, len(target_losses) - 1)
        )
...
def do_test(model, test_data, step):
    logger.info("--------------------------------")
    logger.info("Checking test perplexity...")
    test_loss, test_ppl, test_action_ppl, test_word_ppl = eval_action_ppl(test_data, model)
    logger.info(
        {
            "Step": step,
            "Test ppl": test_ppl,
            "Test action ppl": test_action_ppl,
            "Test word ppl": test_word_ppl,
            "Test loss": test_loss,
        }
    )
    logger.info("--------------------------------")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("{}.log".format(args.save_path)), logging.StreamHandler()],
    )

    # start a new wandb run to track this script
    # wandb.init(
    #     # set the wandb project where this run will be logged
    #     project="syntactic_lm_inductive_bias"
    # )
    ...
    #         "data": args.train_file.split("/")[-4],
    #         "lang": args.train_file.split("/")[-3],
    #         "fold": args.train_file.split("/")[-1].split("-")[0],
    #         "total_epoch": args.num_epochs,
    #         "save_path": args.save_path,
    #         "h_dim": args.h_dim,
    #         "w_dim": args.w_dim,
    #         "use_elman": args.use_elman,
    #     }
    # )
    logger.info(
        {
