...
class FixedInOrderStack(FixedStack):
    def __init__(self, initial_hidden, stack_size, input_size, beam_size=1, use_elman=False):
        super(FixedInOrderStack, self).__init__(initial_hidden, stack_size, input_size, beam_size, use_elman)
    ...
    def do_shift(self, shift_batches, shifted_embs, subword_end_mask):
        ...
        is_begin_subword = subword_end_mask[
            (
                shift_batches[0],
                self.pointer[shift_batches] - 2,
            )
        ] + (
            self.pointer[shift_batches] == 1
        )  # pointer==1 after shift means this is first shifted token  # (shifted_batch_size,)
        begin_subword_shift_batches = tuple([item[is_begin_subword] for item in shift_batches])
        self.last_subword_begin_idx[begin_subword_shift_batches] = self.top_position[begin_subword_shift_batches]
    ...
    def do_nt(self, nt_batches, nt_embs, nt_ids):
        ...
        span_idx_order = (
            torch.arange(max_span_length, device=subword_begin_idx.device)
            .unsqueeze(0)
            .repeat(subword_begin_idx.size(0), 1)
        )
        ...
        for nt_batch in nt_batches:
            nt_batches_for_move += (nt_batch.unsqueeze(-1).expand(-1, max_span_length).reshape(-1)[span_idx_mask],)
        nt_batches_for_move += (
            span_idx,
        )  # reason why not +1 here is due to offset of self.trees relative to self.hiddens.
...
class InOrderStackState(StackState):
    def __init__(self, batch_size, beam_size, device, use_elman=False):
        super(InOrderStackState, self).__init__(batch_size, beam_size, device, use_elman=use_elman)
...
class FixedStackInOrderRNNG(FixedStackRNNG):
    ...
    def build_stack(self, x, batch_size=None):
        ...
        return FixedInOrderStack(initial_hidden, stack_size, self.input_size, use_elman=self.use_elman)

    def new_beam_stack_with_state(self, initial_hidden, stack_size, beam_size):
        stack = FixedInOrderStack(initial_hidden, stack_size, self.input_size, beam_size, use_elman=self.use_elman)
        stack_state = InOrderStackState(
            initial_hidden[0].size(0), beam_size, initial_hidden[0].device, use_elman=self.use_elman
        )
    ...
    def invalid_action_mask(self, beam, sent_lengths, subword_end_mask):
        ...
        finish_mask = ((pointer != sent_lengths) + (nopen_parens != 0) + (top_position != 1)).unsqueeze(
            -1
        ) * finish_mask
